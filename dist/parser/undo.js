...
function readCustomVarInt(bytes, offset) {
    let v = 0n;
    let size = 0;
    while (true) {
...
function decompressScript(nSize, data) {
    if (nSize === 0) {
    ...
    if (nSize === 4 || nSize === 5) {
        ...
        for (let i = 0; i < 32; i++)
            out[34 + i] = parseInt(yBytes.slice(i * 2, i * 2 + 2), 16);
        out[66] = 0xac;
        return out;
    }
    return data.slice(0, nSize - 6);
}
function getSpecialScriptSize(nSize) {
    if (nSize === 0 || nSize === 1)
        return 20;
    if (nSize >= 2 && nSize <= 5)
        return 32;
...
function parseCBlockUndo(bytes, offset, end) {
    const txRecords = [];
...
function parseUndoData(bytes) {
    const blocks = [];
    let offset = 0;
    while (offset + 8 <= bytes.length) {
        const magic = (0, hex_1.readUInt32LE)(bytes, offset);
        if (magic !== UNDO_MAGIC) {
            offset++;
            continue;
        }
        offset += 4;
        const size = (0, hex_1.readUInt32LE)(bytes, offset);
        offset += 4;
