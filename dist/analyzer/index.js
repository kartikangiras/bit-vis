...
const script_1 = require("../parser/script");
const weight_1 = require("./weight");
const timelocks_1 = require("./timelocks");
const segwit_1 = require("./segwit");
const classifier_1 = require("./classifier");
const address_1 = require("./address");
const hex_1 = require("../utils/hex");
function analyzeTransaction(fixture) {
    try {
        const rawTx = (0, transaction_1.parseRawTransaction)(fixture.raw_tx);
        const weightCalc = (0, weight_1.calculateWeight)(rawTx, fixture.raw_tx);
        const prevouts = Array.isArray(fixture.prevouts) ? fixture.prevouts : [];
        const prevoutMap = new Map();
        ...
        for (let i = 0; i < rawTx.inputs.length; i++) {
            ...
            const witnessItems = rawTx.segwit && rawTx.witnesses[i]
                ? rawTx.witnesses[i].map(w => (0, hex_1.bytesToHex)(w))
                : [];
            const scriptType = (0, classifier_1.classifyInputScript)((0, hex_1.bytesToHex)(input.scriptSig), witnessItems, prevout.script_pubkey_hex);
            const address = (0, address_1.generateAddress)(prevout.script_pubkey_hex, fixture.network);
            const relativeTimelock = (0, timelocks_1.analyzeRelativeTimelock)(input.sequence);
            const parsedInput = {
                txid: input.txid,
                vout: input.vout,
                sequence: input.sequence,
                script_sig_hex: (0, hex_1.bytesToHex)(input.scriptSig),
                script_asm: (0, script_1.disassembleScript)((0, hex_1.bytesToHex)(input.scriptSig)),
            ...
            if ((scriptType === 'p2wsh' || scriptType === 'p2sh-p2wsh') && witnessItems.length > 0) {
                ...
                parsedInput.witness_script_asm = (0, script_1.disassembleScript)(witnessScript);
            }
        ...
        for (let i = 0; i < rawTx.outputs.length; i++) {
            ...
            const scriptHex = (0, hex_1.bytesToHex)(output.scriptPubKey);
            const scriptType = (0, classifier_1.classifyOutputScript)(scriptHex);
            const address = (0, address_1.generateAddress)(scriptHex, fixture.network);
        ...
        const isCoinbase = (rawTx.inputs.length === 1 &&
        ...
        const rbfSignaling = (0, timelocks_1.detectRBF)(sequences);
        const locktimeAnalysis = (0, timelocks_1.analyzeLocktime)(rawTx.locktime);
        const txid = (0, transaction_1.computeTxid)(rawTx);
        const wtxid = rawTx.segwit ? (0, transaction_1.computeWtxid)(rawTx, (0, hex_1.hexToBytes)(fixture.raw_tx)) : null;
        const warnings = [];
        if (rbfSignaling) {
        ...
        const segwitSavings = rawTx.segwit
            ? (0, segwit_1.calculateSegwitSavings)(weightCalc.witness_bytes, weightCalc.non_witness_bytes, weightCalc.size_bytes, weightCalc.weight)
            : null;
        const result = {
            ok: true,
